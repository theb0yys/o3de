...
Mono and IL2CPP are separate package families. They do not share target-specific binaries, dependencies, compatibility claims, build results, generated interop, or runtime evidence merely because they implement similar logical operations.

A package manifest declares capabilities and compatibility only. It does not grant process launch, deployment, injection, save mutation, signing, publication, or gameplay mutation. Those actions require separate reviewed work orders, exact-profile validation, explicit permission, execution results, and rollback evidence.
