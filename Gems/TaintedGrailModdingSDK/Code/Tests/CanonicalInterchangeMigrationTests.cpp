...
namespace TaintedGrailModdingSDK::Interchange
{
    ...
    TEST(CanonicalInterchangeMigrationTests, AlreadyCurrentPreservesExactSourceBytes)
    {
        const AZStd::string source(MinimalDocumentsJson);
    ...
    TEST(CanonicalInterchangeMigrationTests, InvalidSourceReturnsNoCandidate)
    {
    ...
    TEST(CanonicalInterchangeMigrationTests, VersionDispatchFailsClosed)
    {
        const MigrationResultV1 downgrade = MigrateCanonicalInterchange(MinimalDocumentsJson, 0);
        EXPECT_EQ(MigrationStatusV1::DowngradeForbidden, downgrade.m_status);
        EXPECT_FALSE(downgrade.HasCandidate());

        const MigrationResultV1 adjacent = MigrateCanonicalInterchange(MinimalDocumentsJson, 2);
        ...
        const MigrationResultV1 future = MigrateCanonicalInterchange(MinimalDocumentsJson, 3);
        EXPECT_EQ(MigrationStatusV1::UnsupportedTargetVersion, future.m_status);
        EXPECT_FALSE(future.HasCandidate());

        AZStd::string unsupportedSource(MinimalDocumentsJson);
        const size_t versionPosition = unsupportedSource.find("\"schema_version\":1");
        ASSERT_NE(AZStd::string::npos, versionPosition);
        unsupportedSource.replace(versionPosition, AZStd::string_view("\"schema_version\":1").size(),
            "\"schema_version\":2");
    ...
    TEST(CanonicalInterchangeMigrationTests, ChangedIdentityCandidateIsSemanticDrift)
    {
        ...
        step.m_sourceDigest = CalculateCanonicalBytesDigestV1(MinimalDocumentsJson);
        step.m_candidateDigest = CalculateCanonicalBytesDigestV1(candidate);

        const auto result = ValidateMigrationCandidateV1(
            MinimalDocumentsJson, candidate, step);
