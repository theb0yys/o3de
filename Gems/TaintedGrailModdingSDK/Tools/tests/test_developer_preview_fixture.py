...
MODULE_PATH = TOOLS_ROOT / "developer_preview_fixture.py"
SPEC = importlib.util.spec_from_file_location("developer_preview_fixture", MODULE_PATH)
assert SPEC and SPEC.loader
fixture = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(fixture)


class DeveloperPreviewFixtureTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temp_root = Path(tempfile.mkdtemp(prefix="tg-preview-fixture-tests-"))
    ...
    def test_generation_is_byte_for_byte_deterministic(self) -> None:
        ...
        for relative_path in left_files:
            self.assertEqual((left / relative_path).read_bytes(), (right / relative_path).read_bytes())
    ...
    def test_generation_refuses_unrelated_non_empty_directory(self) -> None:
        ...
        with self.assertRaisesRegex(fixture.FixtureError, "not empty"):
            fixture.generate_fixture(output)
    ...
    def test_replace_recreates_only_verified_fixture(self) -> None:
        output = self.generate()
        before = (output / fixture.MANIFEST_NAME).read_bytes()
        fixture.generate_fixture(output, replace=True)
        self.assertEqual(before, (output / fixture.MANIFEST_NAME).read_bytes())
        fixture.verify_fixture(output)

    def test_verification_rejects_payload_tampering(self) -> None:
        output = self.generate()
        path = output / fixture.PACK_PATH
        pack = json.loads(path.read_text(encoding="utf-8"))
        pack["DisplayName"] = "Tampered"
        path.write_bytes(fixture.canonical_json_bytes(pack))
        with self.assertRaisesRegex(fixture.FixtureError, "SHA-256 mismatch"):
            fixture.verify_fixture(output)
    ...
    def test_verification_rejects_unexpected_files(self) -> None:
        ...
        with self.assertRaisesRegex(fixture.FixtureError, "missing or unexpected"):
            fixture.verify_fixture(output)
    ...
    def test_manifest_rejects_path_traversal(self) -> None:
        ...
        with self.assertRaisesRegex(fixture.FixtureError, "traversal"):
            fixture.verify_fixture(output)
    ...
    def test_verification_rejects_absolute_or_private_paths(self) -> None:
        ...
        workspace["GameProfiles"][0]["InstallPath"] = "C:\\Users\\Example\\Game"
        workspace_path.write_bytes(fixture.canonical_json_bytes(workspace))
        self._rehash(output, fixture.WORKSPACE_PATH)
        with self.assertRaisesRegex(fixture.FixtureError, "absolute or private path"):
            fixture.verify_fixture(output)
    ...
    def test_source_fingerprint_must_match_source_input(self) -> None:
        ...
        source_input_path.write_bytes(fixture.canonical_json_bytes(source_input))
        self._rehash(output, fixture.SOURCE_INPUT_PATH)
        with self.assertRaisesRegex(fixture.FixtureError, "Source fingerprint"):
            fixture.verify_fixture(output)

    def test_catalog_binding_must_match_workspace_profile(self) -> None:
        output = self.generate()
        catalog_path = output / fixture.CATALOG_PATH
        catalog = json.loads(catalog_path.read_text(encoding="utf-8"))
        catalog["ProfileId"] = "preview.profile.other"
        catalog_path.write_bytes(fixture.canonical_json_bytes(catalog))
        self._rehash(output, fixture.CATALOG_PATH)
        with self.assertRaisesRegex(fixture.FixtureError, "Catalog profile binding"):
            fixture.verify_fixture(output)

    def test_unresolved_relationship_preserves_subject_and_missing_ref(self) -> None:
        output = self.generate()
        catalog = json.loads((output / fixture.CATALOG_PATH).read_text(encoding="utf-8"))
        relationship = next(
            value
            for value in catalog["Relationships"]
            if value["RelationshipId"] == fixture.RELATIONSHIP_UNRESOLVED
        )
        self.assertEqual(relationship["ToRecordId"], "")
        self.assertEqual(relationship["TargetSubjectRef"], fixture.SUBJECT_UNRESOLVED)
        self.assertIn(fixture.SUBJECT_UNRESOLVED, relationship["MissingRefs"])

    def test_fixture_contains_allowed_forbidden_blocked_stale_and_unresolved_states(self) -> None:
        output = self.generate()
        catalog = json.loads((output / fixture.CATALOG_PATH).read_text(encoding="utf-8"))
        records = {record["RecordId"]: record for record in catalog["Records"]}
        self.assertTrue(records[fixture.ITEM_COMPONENT_ID]["AllowedUsages"])
        self.assertTrue(records[fixture.ITEM_RESULT_ID]["ForbiddenUsages"])
        self.assertTrue(records[fixture.STATION_ID]["ForbiddenUsages"])
        self.assertEqual(records[fixture.STATION_ID]["StalenessState"], "stale")
        unresolved = next(
            value
            for value in catalog["Relationships"]
            if value["RelationshipId"] == fixture.RELATIONSHIP_UNRESOLVED
        )
        self.assertTrue(unresolved["TargetSubjectRef"])
        self.assertTrue(unresolved["MissingRefs"])

    def test_all_identities_use_reserved_preview_namespace(self) -> None:
        documents = fixture.template_documents()
        catalog = documents[fixture.CATALOG_PATH]
        for record in catalog["Records"]:
            self.assertTrue(record["RecordId"].startswith("preview."))
            self.assertTrue(record["SubjectRef"].startswith("subject:preview:"))
    ...
    def test_cli_returns_success_for_generate_and_verify(self) -> None:
        ...
        self.assertEqual(fixture.main(["generate", "--output", str(output)]), 0)
        self.assertEqual(fixture.main(["verify", "--output", str(output)]), 0)

    def test_cli_returns_failure_for_invalid_fixture(self) -> None:
        output = self.temp_root / "missing"
        self.assertEqual(fixture.main(["verify", "--output", str(output)]), 1)
