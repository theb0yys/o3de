...
def validate(repo_root: Path) -> None:
    ...
    require(intake.get("license_expression") == "NOASSERTION", "Unverified licence must remain NOASSERTION")
    ...
    for entry in sources:
        require(entry.get("fingerprint_kind") == "git_blob_sha1", "Fingerprint kind must be explicit")
        fingerprint = entry.get("fingerprint", "")
        require(len(fingerprint) == 40 and all(c in "0123456789abcdef" for c in fingerprint),
    ...
    require(len(mono) == 1 and mono[0].get("status") == "live_load_validated",
    ...
    require(len(il2cpp) == 1 and il2cpp[0].get("status") == "blocked",
    ...
    require(len(ready) == 1 and ready[0].get("id") == "runtime-report",
            "Only runtime-report may be consumer-ready")
    require(ready[0].get("consumer") == "mods/Tainted-Diagnostic Tool",
            "Runtime-report consumer binding drifted")
    require(knowledge.get("capabilities") == [
        "read_active_profile", "query_catalog", "submit_candidate_evidence"
    ], "Extension capability declaration escalated or reordered")

    require(fixtures.get("branch_compatibility", {}).get("IL2CPP") == "blocked",
            "Golden fixture must preserve IL2CPP blocking")
    require(fixtures.get("duplicate_registration", {}).get("expected") == "rejected",
            "Golden fixture must reject duplicate registration")
    require(fixtures.get("capabilities", {}).get("unknown") == "rejected",
            "Golden fixture must reject unknown capabilities")
    require(fixtures.get("future_schema", {}).get("expected") == "rejected",
    ...
    require({entry.get("path") for entry in entries} == expected_manifest_paths,
    ...
    framework_manifest = (gem / "Code" / "taintedgrailmoddingsdk_framework_files.cmake").read_text(encoding="utf-8")
    require("Source/TaintedFrameworkKnowledge.cpp" in framework_manifest,
    ...
    for prohibited in ("Tainted.Host.Mono", "Tainted.Host.IL2CPP", "BepInEx", "UnityEngine"):
        require(prohibited not in framework_manifest,
                f"Runtime host leaked into the Editor/Framework build graph: {prohibited}")

    header = (gem / "Code" / "Source" / "TaintedFrameworkKnowledge.h").read_text(encoding="utf-8")
    require("ExtensionAPI::Service&" in header, "First consumer must register through ExtensionAPI")
    for prohibited in ("CatalogDatabase&", "SourceEvidenceRegistry&", "GameProfile&", "WorkspaceModel&"):
        require(prohibited not in header, f"Public knowledge surface exposes mutable authority: {prohibited}")
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print("Tainted Framework knowledge validation passed: pinned provenance, complete classification, canonical fixtures, branch isolation, governed ExtensionAPI registration, and no runtime-host build leakage.")
    return 0
